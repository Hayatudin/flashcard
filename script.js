...
FCCloseButton.click(function (e) {
  ...
  FCForm[0].reset();
  $("#fc_id").val("");
  if (flashcardModal.hasClass("shown")) flashcardModal.removeClass("shown");
});
...
function load_flashcards() {
  ...
  for (var i = 0; i < FCData.length; i++) {
    ...
    fc_item.click(function (e) {
      e.preventDefault();
      var buttonHtml = new RegExp(
        $(this).find("button.card-action")[0].outerHTML,
        "i"
      );

      if (
        $.contains($(this).find("button.card-action")[0], e.target) ||
        $(this).find("button.card-action")[0] == e.target
      )
        return;
